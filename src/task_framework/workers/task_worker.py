...
class TaskWorker(Worker):
    ...
    def run(self):
        ...
        LOGGER.info("Worker started", worker_id=self.worker_id, registry_tasks=len(self.registry.list_tasks()))
        ...
        while self.running:
            ...
            try:
                message = self.task_queue.dequeue(timeout=1)
                if message:                    
                    self._process_message(message)
                    
            except KeyboardInterrupt:
                LOGGER.info("Received interrupt signal, stopping worker", worker_id=self.worker_id)
                break
            except Exception as e:
                self._update_status(WorkerStatus.FAILED)
                LOGGER.error("Error in worker main loop", worker_id=self.worker_id, error=str(e))
                self.metric.counter(
                    'worker.errors', 
                    tags={
    ...
    def _handle_success(self, message: TaskMessage, result: Any, start_time: int, end_time: int):
        ...
        LOGGER.info(f"Task {message.task_id}:{message.task_name} completed successfully",
                        task_name=message.task_name,
    ...
    def _handle_failure(self, message: TaskMessage, error: Exception, start_time: int, end_time: int):
        ...
        LOGGER.error(f"Task {message.task_id}:{message.task_name} execution failed", 
            task_name=message.task_name,
            task_id=message.task_id,
            error=str(error),
            retry_count=message.retry_count,
